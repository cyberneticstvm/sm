                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body table-responsive">
                        <form method="post" action="{{ route('admin.fileUpload') }}" enctype="multipart/form-data">
                            ...
                            <div class="row g-3">
                                <div class="col-lg-6">
                                    <label for="TextInput" class="form-label">File</label>
                ...
                <div class="row g-3">
                    <div class="col-xl-12 col-lg-12 col-md-6 col-sm-12">
                        <div class="card mt-3">
                            ...
                            <div class="card-body table-responsive">
                                <table id="dataTbl" class="table table-bordered table-striped table-sm"><thead><tr><th>SL No</th><th>URL</th><th>Type</th><th>Delete</th></tr></thead><tbody>
                                    @php $c = 1; @endphp
                                    @forelse ($uploads as $key => $file)
                                        <tr>
                                            <td>{{ $c++ }}</td>
                                            <td><a href="{{ public_path().'/storage/'.$file->url }}" target="_blank">{{ $file->url }}</a></td>
                                            <td>{{ $file->type }}</td>
                                            <td>
                                            <form method="post" action="{{ route('admin.file.delete', $file->id) }}">
                                                @csrf 
                                                @method("DELETE")
                                                <button type="submit" class="btn btn-link" onclick="javascript: return confirm('Are you sure want to delete this file?');"><i class="fa fa-trash text-danger"></i></button>
                                            </form>
                                            </td>
                                        </tr>
                                    @empty
